...
// FUNÇÃO PADRONIZADA PARA CRIAR CARD DE RESULTADO - ESTRUTURA UNIFORME PARA TODOS
function createResultCard(peneira) {
    ...
    // Calcular urgência de vagas - SEMPRE mostrar para manter altura uniforme
    const urgencia = getUrgenciaUniforme(peneira);
    const prazoInfo = getPrazoUniforme(peneira);
    ...
    card.innerHTML = `
        ...
        <div class="card-main">
            ...
            <div class="prazo-info ${prazoInfo.class}">
                <span class="prazo-text">${prazoInfo.text}</span>
                <span class="prazo-restante">${prazoInfo.restante}</span>
            </div>
        </div>
        
        <div class="card-actions">
            ${peneira.status === 'aberta' ? `
                <button class="btn-primary" onclick="participarPeneira(${peneira.id})">
...
// Função para participar da peneira
function participarPeneira(peneiraId) {
    ...
    showNotification('Redirecionando para inscrição...', 'info');
    
    // Aqui você pode implementar a lógica real de inscrição
    // Por exemplo, abrir um modal, redirecionar para uma página de inscrição, etc.
    setTimeout(() => {
        showNotification(`Inscrição para ${peneira.titulo} - ${peneira.clube} iniciada!`, 'success');
        
        // Exemplo de redirecionamento ou ação
        // window.open(`/inscricao/${peneiraId}`, '_blank');
        
        // Ou abrir WhatsApp/telefone para contato
        const message = encodeURIComponent(`Olá! Gostaria de me inscrever na peneira: ${peneira.titulo} - ${peneira.clube}. Data: ${formatDate(peneira.data)} às ${peneira.horario}.`);
        const whatsappNumber = peneira.contato.replace(/\D/g, '');
...
// Função para obter urgência de forma uniforme - TODOS os cards terão este bloco
function getUrgenciaUniforme(peneira) {
    if (peneira.status === 'encerrada') {
        return { 
            class: 'encerrada', 
            text: 'Vagas encerradas' 
        };
    }
...
// Função para obter informações de prazo de forma uniforme - TODOS os cards terão este bloco
function getPrazoUniforme(peneira) {
    if (peneira.status === 'encerrada') {
        return {
            class: 'encerrada',
    ...
    return {
        class: '',
        text: `Inscrições até ${formatDateSimple(peneira.prazoInscricao)}`,
        restante: getDiasRestantesSimple(peneira.prazoInscricao)
    };
}
